...
app.service('OfferProvider', function ($resource, utilService, apiService, $q, $http) {
  ...
  this.getExpiredOffersByLocalBusiness = function (localBusinessId) {
    var deferred = $q.defer();
    var OfferResource = $resource(apiService + '/localbusiness/:localBusinessId/offers/expired', {localBusinessId: '@localBusinessId'}, {
      query: {
        method: 'GET',
        isArray: true,
        headers: {token: utilService.apiToken()}
